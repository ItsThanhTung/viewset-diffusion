...
class Lite(LightningLite):
    def run(self, cfg):
        ...
        logger = TensorBoardLogger(cfg.tensorboard.dir, name=cfg.tensorboard.name)
        ...
        for num_epoch in range((n_iter - iteration_start) // len(dataloader) + 1):
            ...
            for data in dataloader:
                ...
                if self.is_global_zero and (iteration + 1) % 10 == 0:
                    if len(losses) == 2:
                        # print(iteration, {"loss_unseen": np.log(losses[0].item() + 1e-8)})
                        # print(iteration, {"loss_seen": np.log(losses[1].item() +1e-8)})
                        logger.experiment.add_scalar("loss_unseen", losses[0].item() + 1e-8, iteration)
                        logger.experiment.add_scalar("loss_seen", losses[1].item() + 1e-8, iteration)

                    # print({"total_loss": np.log(total_loss.item())})
                    # print({"val_loss": np.log(val_loss.item())})
                    logger.experiment.add_scalar("total_loss", total_loss.item(), iteration)
                    logger.experiment.add_scalar("val_loss", val_loss.item(), iteration)
                ...
                if iteration % (cfg.optimization.n_iter // 10) == 0 and self.is_global_zero:
                    ...
                    log_visualisations(diffuser, val_data, noisy_input, iteration, cfg, split="val", logger=logger)

                    noisy_input = torch.randint(0, cfg.model.diffuser.steps, (data["x_in"].shape[0],), ).long()
                    noisy_input = self.to_device(noisy_input)
                    log_visualisations(diffuser, data, noisy_input, iteration, cfg, split="training", logger=logger)
...
def log_visualisations(diffuser, data, noisy_input, iteration, cfg,
                       split, logger):
    vis_images = diffuser.module.module.vis_iteration(data["x_in"] * 2 - 1, 
                        noisy_input,
                        data, 
                        iteration = iteration,
                        split=split)    

    logger.experiment.add_image(f"{split}/vis", vis_images, dataformats="HWC")
    ...
    for cf_guidance_weight in vis_rot_cf_guidances:
        ...
        if cf_guidance_weight == -1.0:
            ...
            for cond_view_idx in range(data["x_cond"].shape[1]):
                ...
                cond_view_result = torch.vstack(rows)
                logger.experiment.add_image(f"{split}/cond_view", cond_view_result, dataformats="HWC")
                ...
                cond_view_gt = torch.vstack(rows)
                logger.experiment.add_image(f"{split}/cond_gt", cond_view_gt, dataformats="HWC")
        ...
        for diffused_view_idx in range(data["x_in"].shape[1]):
            ...
            diffused_view_result = torch.vstack(rows)
            logger.experiment.add_image(f"{split}/diffused_view", diffused_view_result, dataformats="HWC")
        ...
        for rot_idx in range(diffused_views_start+data["x_in"].shape[1], all_images.shape[1]):
            ...
            grid = torch.vstack(rows).permute(2, 0, 1)
            grids.append(grid)
            
        grids = torch.stack(grids)
        logger.experiment.add_video(f"{split}/rot", grids.unsqueeze(0))
