...
CO3D_DATASET_ROOT = "../CO3D_dataset" # Change this line to where CO3D is downloaded
...
class CO3DDataset():
    def __init__(self, cfg,
        ...
        if not os.path.isfile(os.path.join(data_path, "pose_orig_embed_all_sequences_{}.npz".format(dataset_name))):
            print("Preprocessing pose embeddings")
            self.preprocess_pose_embeddings()
            print("Finish reprocessing pose embeddings")
            torch.save(self.pose_orig_embed_all_sequences, \
                         os.path.join(data_path, "pose_orig_embed_all_sequences_{}.npz".format(dataset_name)))
            torch.save(self.pose_dir_embed_all_sequences, \
                         os.path.join(data_path, "pose_dir_embed_all_sequences_{}.npz".format(dataset_name)))
        else:
            print("Loading pose embeddings")
            self.pose_orig_embed_all_sequences = torch.load(os.path.join(data_path, "pose_orig_embed_all_sequences_{}.npz".format(dataset_name)))
            self.pose_dir_embed_all_sequences = torch.load(os.path.join(data_path, "pose_dir_embed_all_sequences_{}.npz".format(dataset_name)))
            print("Finish loading pose embeddings")
